# ESP32-S3 CAM AI 资产管理系统使用指南 v3.4

## 📖 功能概述

本系统实现了基于 **Tag ID** 的资产管理功能，支持**三视图（正面、侧面、顶部）**拍照注册和**智能盘点比对**。系统采用**模块化架构**和**多任务并发设计**，通过 **MobileNetV2 深度学习模型**实现高精度物品识别。**v3.4** 引入 business_executor 业务执行器架构，支持双通道输出（CLI文本/JSON协议）。

### ✨ 核心特性（V3.4完整版）

1. **🆔 Tag ID管理** ⭐v3.2：16位十六进制唯一标识（`0x0001`-`0xFFFF`，支持65,535个资产）
2. **✅ 验证式更新** ⭐v3.2：Tag ID已存在时，拍摄正视图进行身份验证后方可累加数量
3. **🚪 出库模式（分步控制）** ⭐v3.4：按需初始化硬件，用户确认后再拍照，节省资源
4. **📋 资产详细信息** ⭐v2.5：物品名称、存放区域、数量完整管理
5. **🔀 双线程架构** ⭐v2.5：拍摄与推理分离，响应速度提升37倍
6. **🌟 智能盘点模式**：引导式三视图采集 + **加权综合置信度分析**
7. **🗑️ 资产删除功能** ⭐NEW：一键删除资产及其关联图片，支持二次确认
8. **💡 LED状态指示** ⭐NEW：WS2812 RGB LED实时反馈系统状态
9. **🎯 多帧融合** ⭐NEW：每次拍摄采集3帧图像，提升准确率5-8%
10. **📊 混合相似度** ⭐NEW：结合余弦和欧氏距离，提供更鲁棒的匹配评估
11. **🔍 模糊度检测** ⭐NEW v2.6：拉普拉斯方差算法自动过滤模糊图像，提升特征质量
12. **📡 WS63协议支持** ⭐v3.0：JSON格式UART通信，支持主控设备远程调度
13. **🏗️ business_executor架构** ⭐v3.4：统一业务逻辑处理，双通道输出（CLI/JSON）
14. **TF卡存储**：使用 MicroSD/TF 卡存储所有资产数据
15. **实时置信度反馈**：每次推理提供置信度评分，量化识别质量
16. **存储空间监控**：实时监控TF卡使用情况，多级预警机制
17. **🚪 强制退出** ⭐NEW：任何状态下输入 `exit` 立即返回主菜单
...
- [docs/CHANGELOG_v3.4.md](CHANGELOG_v3.4.md) - v3.4版本详细变更记录

---

**最后更新时间**: 2026-06-05  
**版本**: V3.4 (business_executor架构重构版)  
**维护者**: TcXc  
