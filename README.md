# ESP32-S3 CAM AI 资产管理系统 v3.2

> 基于 **ESP32-S3 + OV5640摄像头 + MobileNetV2** 的智能资产管理系统，通过三视图加权综合判断实现高精度物品识别和盘点。**v3.2** 核心升级：从 MAC 地址升级为 16 位 Tag ID（`0x0001`-`0xFFFF`），新增验证式数量累加机制。
