# ESP32-S3 CAM AI 资产管理系统 v3.1

> 基于 **ESP32-S3 + OV5640摄像头 + MobileNetV2** 的智能资产管理系统，通过三视图加权综合判断实现高精度物品识别和盘点。
...
2. **输入MAC地址**：系统启动后输入设备MAC地址（格式：AA:BB:CC:DD:EE:FF）
3. **验证通过**：MAC地址校验成功后自动进入主菜单
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `register` | 资产注册 | `{"cmd":"register","mac":"...","item_name":"扳手"}` |
| `inventory` | 资产盘点 | `{"cmd":"inventory","item_name":"扳手"}` |
| `outbound` | 出库核验 | `{"cmd":"outbound","mac":"...","remove_qty":5}` |
| `capture` | 分步拍摄 | `{"cmd":"capture","view":"front"}` |
| `delete` | 删除资产 | `{"cmd":"delete","mac":"..."}` |
...
- 📗 **协议**: [PROTOCOL.md](docs/PROTOCOL.md) ⭐统一协议文档（v3.1）
