 ...
 * 
 * 支持两种长度（输入时）：
 *   - 短格式：4字符，0x + 2位十六进制，如 0x01
 *   - 标准格式：6字符，0x + 4位十六进制，如 0x0001
 * 存储时统一标准化为6字符格式。
 * 示例：0x01, 0x0001, 0x00AB, 0xABCD, 0xFFFF
 * 范围：0x0001 - 0xFFFF（共65,535个唯一标识）
 * 大小写：不敏感（0xABCD = 0xabcd）
 */
#define TAG_ID_LEN          6       // "0xFFFF" = 6字符（标准格式）
#define TAG_ID_LEN_SHORT    4       // "0x01"  = 4字符（短格式，输入兼容）
 ...
 * 验证规则：
 * - 必须以 "0x" 或 "0X" 开头
 * - 长度必须为 4（短格式，如 0x01）或 6（标准格式，如 0x0001）
 * - 短格式：0x + 2位十六进制字符（0-9, A-F, a-f）
 * - 标准格式：0x + 4位十六进制字符（0-9, A-F, a-f）
